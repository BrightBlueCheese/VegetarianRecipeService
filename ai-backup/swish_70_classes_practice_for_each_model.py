...
model_path = './swish_test_models/weights/'
model_list = os.listdir(model_path)
model_list = [_ for _ in model_list if _.endswith('.weights')]

test_img_path = './swish_test_models/imgs/'
test_img_list = os.listdir(test_img_path)
test_img_list = [_ for _ in test_img_list if _.endswith('.jpg')]



for model in model_list:
    for test_img in test_img_list:
        img_to_detect = cv2.imread(f'{test_img_path}{test_img}')
        ...
        yolo_model = cv2.dnn.readNetFromDarknet(f'./yolov4_swish/darknet_master/yolov4-csp-swish_64_16.cfg', f'{model_path}{model}')
        ...
        ############## NMS Change 3 END ########### 
            ...
            cv2.putText(img_to_detect, predicted_class_label_msg, (start_x_pt, start_y_pt-5), cv2.FONT_HERSHEY_SIMPLEX, 4, box_color, 5)
        ...
        # 모델 이름으로 디렉토리 만든 후 결과 이미지 저장

        model_name = model.split('.weights')[0]
        save_img_path = f'./swish_test_models/model_result/{model_name}'
        ...
        cv2.imwrite(f'{save_img_path}/{test_img}', resized_img)
